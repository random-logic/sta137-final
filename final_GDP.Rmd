...
png("fig/GDP/gdp_ts.png", width = 800, height = 400)
...
png("fig/GDP/boxcox_gdp_ts.png", width = 800, height = 400)
...
png("fig/GDP/diff_gdp_bc.png", width = 800, height = 400)
...
png("fig/GDP/acf_pacf_diff.png", width = 800, height = 400)
par(mfrow = c(1, 2))
...
png("fig/GDP/resid_acf_pacf_model1.png", width = 800, height = 400)
...
# Ljung-Box test plot
png("fig/GDP/ljung_box_pvalues_model1.png", width = 800, height = 400)
...
# Residual ACF and PACF for final model
png("fig/GDP/resid_acf_pacf_final.png", width = 800, height = 400)
par(mfrow = c(1, 2))  # Side-by-side layout
acf(residuals_final, main = "ACF of Final Model Residuals")
pacf(residuals_final, main = "PACF of Final Model Residuals")
par(mfrow = c(1, 1))  # Reset layout
...
# Ljung-Box test plot
png("fig/GDP/ljung_box_pvalues_final.png", width = 800, height = 400)
lb_pvalues <- sapply(1:20, function(lag) Box.test(residuals_final, lag = lag, type = "Ljung-Box")$p.value)
plot(1:20, lb_pvalues, type = "b", pch = 19, ylim = c(0, 1),
     xlab = "Lag", ylab = "Ljung-Box p-value",
     main = "Ljung-Box Test p-values by Lag")
...
# 2. Normality test
# this does not violate model assumptions, but it violates confidence interval assumptions
# Q-Q Plot for Final Model Residuals
png("fig/GDP/qqplot_final_residuals.png", width = 800, height = 400)
...
ggplot() +
  ...
  xlab("Year") + ylab("GDP $") +
  theme_minimal()
ggsave("fig/GDP/forecast_plot.png", width = 8, height = 4)
```
...
ggplot(df_fitted, aes(x = Year)) +
  ...
  ylab("GDP") + xlab("Year") +
  theme_minimal()
ggsave("fig/GDP/fitted_vs_actual.png", width = 8, height = 4)
```